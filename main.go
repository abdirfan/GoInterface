...
func main() {
	...
	// Draw a rectangle
	rect := Rectangle{Point{100, 300}, Point{600, 900}, 1}
	err := rect.draw(&display)
	if err != nil {
		fmt.Println("rect: ", err)
	}
	...
	rect2 := Rectangle{Point{0, 0}, Point{100, 1024}, 2}
	err = rect2.draw(&display)
	if err != nil {
		fmt.Println("rect2: ", err)
	}

	// Attempt to draw a rectangle with an unknown color
	rect3 := Rectangle{Point{0, 0}, Point{100, 1022}, 102}
	err = rect3.draw(&display)
	if err != nil {
		fmt.Println("rect3: ", err)
	...
	// Draw a circle
	circ := Circle{Point{500, 500}, 200, 2}
	err = circ.draw(&display)
	if err != nil {
		fmt.Println("circ: ", err)
	}
	...
	// Draw a triangle
	tri := Triangle{Point{100, 100}, Point{600, 300}, Point{859, 850}, 4}
	err = tri.draw(&display)
	if err != nil {
		fmt.Println("tri: ", err)
	}
